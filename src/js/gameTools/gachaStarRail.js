...
function initSettingsMenu() {
  ...
  document.getElementById('menu-clear-url-cache').addEventListener('click', async () => {
    closeMenu();
    try {
      const result = await window.electronAPI.invoke('clear-starRail-url-cache');
...
function openHidePoolsModal() {
  ...
  const STORAGE_KEY = 'starRail_hidden_pools';
...
function getHiddenPools() {
  ...
  try {
    ...
    localStorage.removeItem('starRail_hidden_pools');
    return new Set();
  }
}
...
async function openDeleteByTimeModal() {
  ...
  // 拉取记录并过滤当前 uid
  const all = await window.electronAPI.invoke('get-starRail-gacha-records');
